...
import knife4jConfig from '@/config';
...
function SwaggerBootstrapUi(options) {
  ...
  //  swagger请求api地址
  var swaggerResourcesUrl = knife4jConfig.apiUrl.swaggerResources;
  var v3ApiDocsUrl = knife4jConfig.apiUrl.v3ApiDocs;
  if (this.springdoc) {
    const path = window.location.pathname;
    const index = path.lastIndexOf('/');
    const basePath = path.length == index + 1 ? path : path.substring(0, index);
    if (basePath != '' && basePath != '/') {
      this.url = options.url || basePath + '/' + v3ApiDocsUrl + '/swagger-config';
    } else {
      this.url = options.url || v3ApiDocsUrl + '/swagger-config';
    }
  } else {
    this.url = options.url || swaggerResourcesUrl;
  }
  ...
  this.configUrl = options.configUrl || (knife4jConfig.apiUrl.swaggerResources + '/configuration/ui');
  ...
  this.extUrl = knife4jConfig.apiUrl.v2ApiDocs;
  this.ext3Url = knife4jConfig.apiUrl.v3ApiDocs;
  // 验证增强有效地址
  this.validateExtUrl = '';
    ...
    for (var name in definitions) {
      if (name == modelName) {
        ...
        if (KUtils.checkUndefined(value)) {
          ...
          if (value.hasOwnProperty('enum') && KUtils.arrNotEmpty(value['enum'])) {
            swud.enum = value['enum'];
          }
          // 判断是否有required属性
              ...
              if (propobj.hasOwnProperty('type')) {
                ...
                if (propobj.hasOwnProperty('$ref')) {
                  ...
                  if (regex.test(ref)) {
                    ...
                    spropObj.refType = refType;
                    // 检查被引用类型是否是枚举
                    if (KUtils.checkUndefined(definitions[refType])) {
                      var refDef = definitions[refType];
                      if (refDef.hasOwnProperty('enum') && KUtils.arrNotEmpty(refDef['enum'])) {
                        spropObj.enum = refDef['enum'];
                        spropObj.description = KUtils.enumAvalibleLabel(that.i18nInstance, refDef['enum'], spropObj.description);
                      }
                    }
                    // 这里需要递归判断是否是本身,如果是,则退出递归查找
                    var globalArr = new Array();
...
SwaggerBootstrapUi.prototype.getOriginalDefinitionByName = function (name, definitions) {
  ...
  for (var key in definitions) {
    if (key == name) {
      ...
      // 复制枚举属性，支持引用类型的枚举定义
      if (definitions[key].hasOwnProperty('enum') && KUtils.arrNotEmpty(definitions[key]['enum'])) {
        def['enum'] = definitions[key]['enum'];
      }
      // 复制 type 属性
      if (definitions[key].hasOwnProperty('type')) {
        def['type'] = definitions[key]['type'];
      }
      break;
    }
  }
...
SwaggerBootstrapUi.prototype.getSwaggerModelRefType = function (propobj, oas2) {
  ...
  if (propobj.hasOwnProperty('type')) {
    var type = propobj['type'];
    // 处理 .NET 特殊格式：type 字段本身是一个对象包含 $ref
    // 例如: { "type": { "$ref": "#/components/schemas/ModelTypeEnum", "refType": "ModelTypeEnum" } }
    if (typeof type === 'object' && type !== null) {
      // 优先使用 refType 字段
      if (type.hasOwnProperty('refType') && KUtils.strNotBlank(type['refType'])) {
        refType = type['refType'];
      } else if (type.hasOwnProperty('$ref')) {
        var ref = type['$ref'];
        var regex = new RegExp(KUtils.oasmodel(oas2), 'ig');
        if (regex.test(ref)) {
          refType = RegExp.$1;
    ...
    for (var key in definitions) {
      if (key == deepDef.name) {
        ...
        if (KUtils.checkUndefined(def)) {
          ...
          // 处理枚举类型的 Definition（如 .NET 的枚举 Schema 引用）
          else if (def.hasOwnProperty('enum') && KUtils.arrNotEmpty(def['enum'])) {
            // 如果父参数还没有描述，才设置枚举信息
            if (!KUtils.strNotBlank(parentRefp.description)) {
              var enumDescription = KUtils.propValue('description', def, '');
              parentRefp.description = KUtils.enumAvalibleLabel(that.i18nInstance, def['enum'], enumDescription);
            }
            parentRefp.enum = def['enum'];
          }
        }
      }
    }
            ...
            if (def.hasOwnProperty('properties')) {
              ...
              props.forEach(function (p) {
                ...
                if (!KUtils.checkParamArrsExists(swaggerResp.responseParameters, resParam)) {
                  ...
                  if (p.enum != undefined && p.enum != null && KUtils.arrNotEmpty(p.enum)) {
                    resParam.enum = p.enum;
                    resParam.description = KUtils.enumAvalibleLabel(that.i18nInstance, p.enum, resParam.description);
                  }
                  // 先检查 refType 是否存在，如果存在则检查是否是枚举类型
                  if (p.refType != null && !KUtils.checkIsBasicType(p.refType)) {
                    ...
                    if (deepDef != null && deepDef.enum != undefined && KUtils.arrNotEmpty(deepDef.enum)) {
                      resParam.enum = deepDef.enum;
                      resParam.description = KUtils.enumAvalibleLabel(that.i18nInstance, deepDef.enum, resParam.description);
                    }
...
SwaggerBootstrapUi.prototype.assembleParameter = function (m, swpinfo) {
  ...
  if (m.hasOwnProperty('enum')) {
    ...
    minfo.description = KUtils.enumAvalibleLabel(that.i18nInstance, m.enum, minfo.description);
    ...
    if (schemaType == 'array') {
      ...
      if (schemaObject.hasOwnProperty('$ref')) {
        ...
        if (def != null) {
          ...
          if (def.enum != undefined && def.enum != null && KUtils.arrNotEmpty(def.enum)) {
            minfo.enum = def.enum;
            minfo.description = KUtils.enumAvalibleLabel(that.i18nInstance, def.enum, minfo.description);
          }
        }
      } else {
        ...
        if (schemaObject.hasOwnProperty('additionalProperties')) {
          ...
          if (KUtils.arrNotEmpty(_enumArray)) {
            ...
            minfo.description = KUtils.enumAvalibleLabel(that.i18nInstance, _enumArray, minfo.description);
          }
          minfo.value = '';
        }
      }
      ...
      if (!KUtils.checkParamArrsExists(swpinfo.responseRefParameters, refParam)) {
        ...
        if (def.hasOwnProperty('properties')) {
          ...
          props.forEach(p => {
            ...
            if (p.enum != undefined && p.enum != null && KUtils.arrNotEmpty(p.enum)) {
              refp.enum = p.enum;
              refp.description = KUtils.enumAvalibleLabel(that.i18nInstance, p.enum, refp.description);
            }
